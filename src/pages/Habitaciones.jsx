...
export const Habitaciones = () => {
  ...
  const handleCrearCama = async () => {
    await crearCama({
      ...
      fechaCama: camaData.fechaCama,
      categoria: { idCategoriaCama: parseInt(camaData.idCategoriaCama) },
    });
    alert("Cama creada");
    cargarDatos();
  };
  ...
  return (
    <div>
      ...
      <h3>Crear Categoría de Habitación</h3>
      <input
        placeholder="Nombre"
        onChange={(e) =>
          setCategoriaHabitacionData({
            ...categoriaHabitacionData,
      ...
      />
      <button onClick={handleCrearCategoriaHabitacion}>
        Crear Categoría Habitación
      </button>

      <h3>Crear Categoría de Cama</h3>
      <input
        placeholder="Tipo"
        onChange={(e) =>
          setCategoriaCamaData({ ...categoriaCamaData, tipo: e.target.value })
        }
      />
      <input
        placeholder="Medidas"
        onChange={(e) =>
          setCategoriaCamaData({
            ...categoriaCamaData,
      ...
      <input
        placeholder="Color"
        onChange={(e) =>
          setCategoriaCamaData({ ...categoriaCamaData, color: e.target.value })
        }
      />
      <button onClick={handleCrearCategoriaCama}>Crear Categoría Cama</button>
      ...
      <select
        onChange={(e) =>
          setCamaData({ ...camaData, idCategoriaCama: e.target.value })
        }
      >
        <option value="">Seleccione Categoría de Cama</option>
        {categoriasCama.map((c, i) => (
          <option key={i} value={c.idCategoriaCama}>
            {c.tipo}
          </option>
        ))}
      </select>
